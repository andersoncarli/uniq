...
public:
  ...
  BigInteger(const std::string s, int base=10, std::string map=DIGITS) : BigCardinal(), signal(1) {
    ...
    bool negative = (s[0] == '-');
    std::string num_str = negative ? s.substr(1) : s;
    if(num_str.empty()) {
      digits_.clear();
      digits_.push_back(BigDigit(0));
      signal = 1;
  ...
  std::string format(int base=10, std::string map=DIGITS) const {
    ...
    if(result == "-" + std::string(1, map[0])) result = std::string(1, map[0]);
    return result;
  }
...
std::ostream& operator<<(std::ostream& os, const BigInteger& n) { return os << n.format(); }
